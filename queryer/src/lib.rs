...
pub async fn query<T: AsRef<str>>(sql: T) -> Result<DataSet> {
    let ast = Parser::parse_sql(&AdkDialect, sql.as_ref())?;

    if ast.len() != 1 {
        return Err(anyhow!("Only support one query at the moment"));
    }

    let sql = &ast[0];

    // 整个SQL AST转换成我们定义的Sql结构的细节都埋藏在try_into()中
    // 我们只需关注数据结构的使用，怎么转换可以之后需要的时候才关注，这是关注点分离，是我们控制软件复杂度的法宝。
    let Sql {
        source,
        condition,
        selection,
